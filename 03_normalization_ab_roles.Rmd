...
traits.bdd <- read.csv2("./Traits/BDD_MarieCaroline/BDD_Poissons côtiers_MCH.csv")
...
DATA.sp.list.site.HC <- DATA.sp.HC %>%
  ...
  #filter(idSINPRegroupement_tran=="{F4271AE9-C6BF-4E9D-AF8A-179064A31D1C}") %>%
  group_by(idSINPRegroupement_tran) %>%
...
DATA.sp.BHC.tmp <- DATA.sp.BHC %>%
  group_by(idSINPRegroupement_tran, Species) %>%
  dplyr::summarize(abundance = sum(abondance)) 

sp.0 <- DATA.sp.BHC.wide %>%
  group_by(Species) %>%
  summarize(sum.sp=sum(abundance)) %>%
  filter(sum.sp==0) %>%
  filter(Species!="No_species") %>%
  pull(Species)

# Remove species which have been dropped to 0 with the normalization step, excpet no_species
DATA.sp.BHC.noSp0 <- DATA.sp.BHC.tmp %>%
  filter(!Species %in% sp.0) %>%
  rename(nomCite = Species,
         abondance.tran.norm = abundance)
...
TR.agg.sp <- DATA.sp.BHC.noSp0 %>%
  full_join(DATA.sp.info, by="idSINPRegroupement_tran") %>%
  # in case the normalization removed the few occurences of a transect, we need to modify this transect into "no_species"
  mutate(nomCite = as.character(nomCite)) %>%
  mutate(nomCite2 = case_when(is.na(nomCite) ~ "No_species",
                             TRUE ~ nomCite)) %>%
  mutate(abondance.tran.norm2 = case_when(is.na(abondance.tran.norm) ~ 0,
                                         TRUE ~ abondance.tran.norm)) %>%
  select(-nomCite, -abondance.tran.norm) %>%
  rename(nomCite = nomCite2,
         abondance.tran.norm = abondance.tran.norm2)
...
rm(DATA.sp.size.BHC.ab)
DATA.sp.size.BHC.ab <- DATA.sp.size.BHC %>%
  group_by(idSINPRegroupement_tran, nomCite_size) %>%
  dplyr::summarize(abundance = sum(abondance)) 

sp.size.0 <- DATA.sp.size.BHC.ab %>%
  group_by(nomCite_size) %>%
  summarize(sum.sp=sum(abundance)) %>%
  filter(sum.sp==0) %>%
  filter(!str_detect(nomCite_size, "No_species")) %>%
  pull(nomCite_size)

# Remove species which have been dropped to 0 with the normalization step, excpet no_species
DATA.sp.size.BHC.ab.noSp0 <- DATA.sp.size.BHC.ab %>%
  filter(!nomCite_size %in% sp.size.0) %>% 
  rename(abondance.tran.norm = abundance)
...
TR.agg.sp.size <- DATA.sp.size.BHC.ab.noSp0 %>%
  ...
  # in case the normalization removed the few occurences of a transect, we need to modify this transect into "no_species"
  mutate(nomCite_size = as.character(nomCite_size)) %>%
  mutate(nomCite_size2 = case_when(is.na(nomCite_size) ~ "No_species",
                             TRUE ~ nomCite_size)) %>%
  mutate(abondance.tran.norm2 = case_when(is.na(abondance.tran.norm) ~ 0,
                                         TRUE ~ abondance.tran.norm)) %>%
  select(-nomCite_size, -abondance.tran.norm) %>%
  rename(nomCite_size = nomCite_size2,
         abondance.tran.norm = abondance.tran.norm2) 
...
rm(DATA.sp.size.BHC.biom)
DATA.sp.size.BHC.biom <- DATA.sp.size.BHC.2 %>%
  group_by(idSINPRegroupement_tran, nomCite2) %>%
  dplyr::summarize(Sum.biomass = sum(biomass)) 

sp.biom.0 <- DATA.sp.size.BHC.biom %>%
  group_by(nomCite2) %>%
  summarize(sum.sp=sum(Sum.biomass)) %>%
  filter(sum.sp==0) %>%
  filter(nomCite2!="No_species") %>%
  pull(nomCite2)

# Remove species which have been dropped to 0 with the normalization step, excpet no_species
DATA.sp.size.BHC.biom.noSp0 <- DATA.sp.size.BHC.biom %>%
  filter(!nomCite2 %in% sp.0) %>%
  rename(nomCite = nomCite2,
         biomasse.tran.norm = Sum.biomass)
...
TR.agg.sp.size.biom <- DATA.sp.size.BHC.biom.noSp0 %>%
  full_join(DATA.sp.size.info, by="idSINPRegroupement_tran") %>%
  # in case the normalization removed the few occurences of a transect, we need to modify this transect into "no_species"
  mutate(nomCite = as.character(nomCite)) %>%
  mutate(nomCite2 = case_when(is.na(nomCite) ~ "No_species",
                             TRUE ~ nomCite)) %>%
  mutate(biomasse.tran.norm2 = case_when(is.na(biomasse.tran.norm) ~ 0,
                                         TRUE ~ biomasse.tran.norm)) %>%
  select(-nomCite, -biomasse.tran.norm) %>%
  rename(nomCite = nomCite2,
         biomasse.tran.norm = biomasse.tran.norm2)
...
# Sum the abundances found by role at the transect level for H et B
DATA.sp.troncon.BHC.tmp <- DATA.sp.troncon.BHC %>%
  group_by(id_tran_sec, Species) %>%
  dplyr::summarize(abundance = sum(abondance)) 

dim(DATA.sp.troncon.BHC.tmp)

sp.sec.0 <- DATA.sp.troncon.BHC.tmp %>%
  group_by(Species) %>%
  summarize(sum.sp=sum(abundance)) %>%
  filter(sum.sp==0) %>%
  filter(Species!="No_species") %>%
...
# Remove species which have been dropped to 0 with the normalization step, excpet no_species
DATA.sp.troncon.BHC.noSp0 <- DATA.sp.troncon.BHC.tmp %>%
  filter(!Species %in% sp.0) %>%
  rename(nomCite = Species,
         abondance.sec.norm = abundance)
...
TR.agg.sp.troncon <- DATA.sp.troncon.BHC.noSp0 %>%
  full_join(DATA.sp.troncon.info, by="id_tran_sec") %>%
  # in case the normalization removed the few occurences of a transect, we need to modify this transect into "no_species"
  mutate(nomCite = as.character(nomCite)) %>%
  mutate(nomCite2 = case_when(is.na(nomCite) ~ "No_species",
                             TRUE ~ nomCite)) %>%
  mutate(abondance.sec.norm2 = case_when(is.na(abondance.sec.norm) ~ 0,
                                         TRUE ~ abondance.sec.norm)) %>%
  select(-nomCite, -abondance.sec.norm) %>%
  rename(nomCite = nomCite2,
         abondance.sec.norm = abondance.sec.norm2)
...
TR.agg.sp %>% as.data.frame()  %>% summarize(sum(abondance.tran.norm))
TR.agg.sp.size %>% as.data.frame() %>% summarize(sum(abondance.tran.norm))
```
...
TR.agg.sp.size.PMG <- TR.agg.sp.size %>%
  ...
  group_by(nomCite, size.class.mode, idSINPRegroupement_tran) %>%
...
save(TR.agg.sp.size.PMG, file = "./Data/TR_agg_sp_size_PMG.RData")
